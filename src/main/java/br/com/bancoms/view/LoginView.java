...
public class LoginView extends StackPane {
    ...
    public LoginView(MainView mainView) {
        ...
        double insets = SistemaCaixaEletronico.METRICS.getPX(0.015);

        setPickOnBounds(false);

        Group group = new Group();
        VBox loginBox = new VBox();
        loginBox.setSpacing(SistemaCaixaEletronico.METRICS.getPX(0.015));
        ...
        labelLoginTitulo.setFont(Font.font(SistemaCaixaEletronico.METRICS.getPX(0.012)));
        ...
        labelNumeroConta.setFont(Font.font(SistemaCaixaEletronico.METRICS.getPX(0.007)));

        Label labelSenhaConta = new Label("Senha");
        labelSenhaConta.getStyleClass().add("labelStyleDark");
        labelSenhaConta.setFont(Font.font(SistemaCaixaEletronico.METRICS.getPX(0.007)));

        fieldNumeroConta = new TextField();
        fieldNumeroConta.getStyleClass().add("fieldStyle");
        fieldNumeroConta.setFont(Font.font(SistemaCaixaEletronico.METRICS.getPX(0.007)));


        fieldSenha = new PasswordField();
        fieldSenha.setFont(Font.font(SistemaCaixaEletronico.METRICS.getPX(0.007)));
